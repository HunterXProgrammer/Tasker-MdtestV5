...
start_line="$(($(grep -nm 1 -F 'linkingCode, err := cli.PairPhone(args[0], true)' whatsmeow/mdtest/main.go | sed 's/:.*//')-1))"

sed -i '/linkingCode, err := cli.PairPhone(args\[0\], true)/,/fmt.Println("Linking code:", linkingCode)/d' whatsmeow/mdtest/main.go
